...
import {
  ...
  watchStoreRichOrders, setOrderStatus,
} from '../lib/db';
...
type Tab = 'overview' | 'menu' | 'deals' | 'orders' | 'profile';
    ...
    <div className="min-h-screen w-full flex bg-[#080808] text-stone-100">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-6 py-8">
          {tab === 'overview' && <Overview store={store} menu={menu} deals={deals} orders={orders} />}
...
function Overview({ store, menu, deals, orders }: { store: StoreDoc | null; menu: MenuItemDoc[]; deals: DealDoc[]; orders: any[] }) {
...
function Orders({ orders, flash }: { orders: any[]; flash: (m: string) => void }) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-black text-white mb-1">Live Orders</h1>
      <p className="text-sm text-stone-500 mb-6">Updates in real time from Firebase as customers order.</p>
      {sorted.length === 0 ? (
        <div className="glass-soft rounded-2xl p-10 text-center text-stone-500 text-sm">No orders yet.</div>
      ) : (
        <div className="space-y-3">
          {sorted.map(o => {
            ...
            return (
              <div key={o.id} className="glass rounded-2xl p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="text-sm font-black text-white">Order #{String(o.id).slice(-6).toUpperCase()}</p>
                    <p className="text-[11px] text-stone-500">{o.createdAt ? new Date(o.createdAt).toLocaleString() : ''} · {o.deliveryType || 'delivery'}</p>
                  </div>
                  <p className="font-black text-white">{money(o.finalTotal ?? o.total)}</p>
                </div>
